    ...
    <div class="pt-16 md:pt-42 px-10 pb-32">
      ...
      {#if unlockedMenu.length === 0}
        <div class="flex flex-col items-center justify-center mt-20 text-center">
          ...
          <h2 class="text-4xl text-[#380730]">Kamu belum punya resep apapun, mulai masak!</h2>
          <a href="/cook" class="mt-6 px-8 py-4 bg-[#38072d] text-white rounded-full pgm hover:scale-110 transition-all">
            Ke dapur
          </a>
        </div>
      {:else}
<div class="grid grid-cols-2 md:grid-cols-5 gap-6 mt-10"> {#each unlockedMenu as item}
    <a href="/menu/recipes/{item.id}" class="block no-underline"> <div class="bg-[#fef6fe] p-5 rounded-xl border-4 border-[#6a1d59] hover:scale-105 transition-all duration-300 shadow-xl h-full flex flex-col">
        ...
        <div class="mt-4 flex-grow">
          <div class="pix text-[10px] bg-[#6a1d59] text-white inline-block px-2 py-0.5 rounded mb-2">
            {item.rarity}
          </div>

          <h2 class="text-sm md:text-xl pgm uppercase leading-tight">{item.name[$language]}</h2>
          
          <p class="text-lg font-bold mt-2 text-[#a83d93]">
            {#if $currency === 'id'}
